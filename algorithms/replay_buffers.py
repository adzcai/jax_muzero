...
class UniformBuffer(object):
    ...
    def extend(self, timesteps: Any):
        if self._timestep_storage is None:
            sample_timestep = jax.tree.map(lambda t: t[0], timesteps)
        ...
        jax.tree.map(lambda a, x: assign(a, indices, x), self._timestep_storage, timesteps)
    ...
    def sample(self, batch_size: int):
        ...
        trajectories = jax.tree.map(lambda a: a[all_indices], self._timestep_storage)
        return trajectories
    ...
    def _preallocate(self, item):
        return jax.tree.map(lambda t: np.empty((self._max_size,) + t.shape, t.dtype), item)
